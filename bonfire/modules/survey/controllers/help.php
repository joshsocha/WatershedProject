...
class help extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Survey.Help.View');
		$this->load->model('survey_model', null, true);
		$this->lang->load('survey');
		
		Template::set_block('sub_nav', 'help/_sub_nav');
	}
	...
	*/
	...
	{
		$this->auth->restrict('Survey.Help.Create');
		...
		if ($this->input->post('save'))
		{
			if ($insert_id = $this->save_survey())
			{
				...
				Template::redirect(SITE_AREA .'/help/survey');
			}
			else
			{
	...
	*/
	public function edit()
	{
		...
		if (empty($id))
		{
			...
			redirect(SITE_AREA .'/help/survey');
		}

		if (isset($_POST['save']))
		{
			$this->auth->restrict('Survey.Help.Edit');
		...
		{
			$this->auth->restrict('Survey.Help.Delete');
